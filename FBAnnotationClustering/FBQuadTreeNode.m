...
FBBoundingBox FBBoundingBoxMake(CGFloat x0, CGFloat y0, CGFloat xf,
                                CGFloat yf) {
  FBBoundingBox box;
  box.x0 = x0;
  box.y0 = y0;
...
FBBoundingBox FBBoundingBoxForMapRect(MKMapRect mapRect) {
  ...
  CLLocationCoordinate2D botRight = MKCoordinateForMapPoint(
...
MKMapRect FBMapRectForBoundingBox(FBBoundingBox boundingBox) {
  MKMapPoint topLeft = MKMapPointForCoordinate(
      CLLocationCoordinate2DMake(boundingBox.x0, boundingBox.y0));
  MKMapPoint botRight = MKMapPointForCoordinate(
      CLLocationCoordinate2DMake(boundingBox.xf, boundingBox.yf));
...
BOOL FBBoundingBoxContainsCoordinate(FBBoundingBox box,
  ...
  BOOL containsX =
      box.x0 <= coordinate.latitude && coordinate.latitude <= box.xf;
  BOOL containsY =
      box.y0 <= coordinate.longitude && coordinate.longitude <= box.yf;
  return containsX && containsY;
}

BOOL FBBoundingBoxIntersectsBoundingBox(FBBoundingBox box1,
                                        FBBoundingBox box2) {
  return (box1.x0 <= box2.xf && box1.xf >= box2.x0 && box1.y0 <= box2.yf &&
          box1.yf >= box2.y0);
}
