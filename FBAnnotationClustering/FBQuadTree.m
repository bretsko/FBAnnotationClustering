...
- (id)init {
  self = [super init];
  if (self) {
    self.rootNode = [[FBQuadTreeNode alloc]
        initWithBoundingBox:FBBoundingBoxForMapRect(MKMapRectWorld)];
  }
  return self;
}
...
- (BOOL)removeAnnotation:(id<MKAnnotation>)annotation
                fromNode:(FBQuadTreeNode *)node {
  if (!FBBoundingBoxContainsCoordinate(node.boundingBox,
...
- (BOOL)insertAnnotation:(id<MKAnnotation>)annotation
  ...
  if ([self insertAnnotation:annotation toNode:node.northEast])
    return YES;
  if ([self insertAnnotation:annotation toNode:node.northWest])
    return YES;
  if ([self insertAnnotation:annotation toNode:node.southEast])
    return YES;
  if ([self insertAnnotation:annotation toNode:node.southWest])
    return YES;
...
- (void)enumerateAnnotationsInBox:(FBBoundingBox)box
                       usingBlock:(void (^)(id<MKAnnotation>))block {
...
- (void)enumerateAnnotationsUsingBlock:(void (^)(id<MKAnnotation>))block {
  [self enumerateAnnotationsInBox:FBBoundingBoxForMapRect(MKMapRectWorld)
                         withNode:self.rootNode
                       usingBlock:block];
}

- (void)enumerateAnnotationsInBox:(FBBoundingBox)box
                         withNode:(FBQuadTreeNode *)node
                       usingBlock:(void (^)(id<MKAnnotation>))block {
